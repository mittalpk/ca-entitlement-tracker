...
## 1. Risk register (Reviewed & Verified 2026-08-17 — Phase 6 Release Readiness Gate)

| Risk ID | Risk description | Likelihood | Impact | Risk rating | Mitigation | Owner | Status (as of 2026-08-17) |
|---|---|---|---|---|---|---|---|
| R-001 | LLM hallucinates a financial figure not in locked inputs | Low | Critical | High | Basic LLM Chain node (`lmChatOpenAi` sub-node); prompt locked variables; 100% verified in `tests/kpi-benchmark.js` | Technology / Workflow Engineer | **CLOSED — Verified** |
| R-002 | Record-date position substituted with live position (coding error in formula node) | Low | Critical | High | `positionAsOfRecordDate` field constraint enforced; verified in TC-001–TC-008 unit test suite | Workflow Engineer | **CLOSED — Verified** |
| R-003 | BREACH test case (TC-004) not exercised before production | Low | High | High | TC-004 mandatory test executed and passed in `tests/unit-tests.js` | Workflow Engineer | **CLOSED — Verified** |
| R-004 | Prompt injection via optionDetails[].description | Low | High | Medium | Input sanitisation in Validation Code node before prompt assembly; verified in `tests/malformed-input-tests.js` | Workflow Engineer | **CLOSED — Verified** |
| R-005 | Google Sheets audit trail write race condition under parallel executions | Low (MVP) | Medium | Low | Accepted for MVP synthetic testing; database migration path specified for production | Data Governance Lead | **OPEN — Controlled (MVP Scope)** |
| R-006 | LLM provider model deprecated mid-year | Medium | Medium | Medium | Model version pinned to `gpt-4o-mini`; fallback template verified in `tests/recovery-drill.js` | Workflow Engineer | **CLOSED — Verified** |
| R-007 | Portfolio uniqueness overlap risk (public template similarity) | Low | Medium | Medium | 5 named formulas, record-date constraint, 4-tier escalation engine verified distinct | Corporate Actions Operations Lead | **CLOSED — Documented** |
| R-008 | Credential exported in workflow JSON (security risk) | Low | High | High | Header Auth credential reference (`credential-webhook-secret`) used; zero plaintext secret literals | Workflow Engineer | **CLOSED — Verified (`BK1-ISS-002`)** |
