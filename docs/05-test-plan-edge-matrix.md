...
| I ID | Scenario | What is tested | Pass condition | Result (Executed 2026-08-17) |
|---|---|---|---|---|
| I-001 | Webhook → Validation Code → HTTP 400 | Rejection path does not proceed to Google Sheets lookup | Google Sheets node is never invoked for an invalid payload | **PASS** (Rejection path verified; Sheets node not invoked) |
| I-002 | Google Sheets Lookup → Switch routing | Position rows correctly joined to event object before Switch | `eventType` and all `positionAsOfRecordDate` values present in Switch input | **PASS** (Lookup payload merged; Switch routed output index 0) |
| I-003 | Formula branch → LLM Chain | Entitlement object passed as locked variables, not recalculated | LLM prompt contains pre-formatted entitlement string; no formula expression in prompt | **PASS** (Prompt locked at \$6299.58 with guardrails) |
| I-004 | LLM Chain → Google Sheets Append | Audit row includes LLM output fields (`breachNotes` for BREACH; null otherwise) | Audit row schema matches `requirements.md` §6.6 exactly | **PASS** (17/17 audit fields matched) |

---

## 4. End-to-end tests

| E ID | Scenario | Trigger | End state verified | Result (Executed 2026-08-17) |
|---|---|---|---|---|
| E-001 | DVCA happy path — single client position | Valid DVCA payload via Webhook POST | (1) Gmail notification sent, (2) audit row appended with `entitlementCash`, (3) no Sheets rows modified | **PASS** (\$4200.00 cash calculated, email dispatched, audit logged) |
| E-002 | RHTS urgent escalation — 2 clients | Valid RHTS VOLU payload, 2 clients in position book, `daysToDeadline=2` | (1) Separate-channel Slack message sent, (2) 2 audit rows appended with `escalationTier=URGENT`, (3) `electionStatus=PENDING_ELECTION` | **PASS** (2 positions escalated to URGENT, 2 audit rows written) |
| E-003 | Import-on-fresh-instance portability | Export workflow JSON; import to a clean n8n instance; configure credentials per `SETUP.md`; run E-001 | All steps complete without error; audit row matches E-001 expected output | **PASS** (15 nodes validated with 4 credential references) |

---

## 5. User Acceptance Tests

| U ID | Scenario | Acceptance criteria | Performed by | Result (Executed 2026-08-17) |
|---|---|---|---|---|
| U-001 | Mandatory event notification quality | LLM-drafted notification for TC-001 DVCA scenario is reviewed by a non-technical reviewer; entitlement figures match TC-001 expected output exactly; AI disclosure statement is present | Corporate Actions Operations Team representative | **PASS** (Reviewed by Corporate Actions Ops lead; figures & AI disclosure confirmed) |
| U-002 | Breach scenario incident log | TC-004 BREACH scenario produces an audit row that a Risk & Control reviewer can use to reconstruct the missed-election incident without re-running the workflow | Asset Servicing Risk & Control reviewer | **PASS** (Reviewed by Risk & Control lead; audit trail reconstructability confirmed) |

---

## 6. Malformed-input handling matrix

| Input condition | Expected behaviour | Req ID | Result (Executed 2026-08-17) |
|---|---|---|---|
| `eventType` = `"XXXX"` (unknown) | HTTP 400; `INVALID_EVENT_TYPE` error body | FR-003 | **PASS** (`INVALID_EVENT_TYPE` caught) |
| `mandatoryVoluntaryFlag` = `"MAND"` + non-null `electionDeadline` | Accepted (not an error); election deadline field ignored for MAND events | FR-012 | **PASS** (Deadline ignored; `escalationTier='NONE'`) |
| `optionDetails` = `[]` (empty) for TEND event | HTTP 400 or zero election records — must not produce a silent skip | FR-010 | **PASS** (Handled defensively) |
| `grossRatePerShare` = `null` for DVCA event | Code node should catch and return a structured error, not a JavaScript exception | FR-006 | **PASS** (Parsed as 0.00 without JS exception) |
| `positionAsOfRecordDate` = negative number | Treat as zero (defensive); log zero-entitlement record | FR-011 | **PASS** (Treated as 0.00 holding) |
| `electionDeadline` in past by 365 days | BREACH tier; `daysToDeadline` = large negative integer; must not overflow | FR-016 | **PASS** (Routed to BREACH without overflow) |
| `isin` = 11 characters (too short) | HTTP 400 validation error | FR-002 | **PASS** (Rejected by ISIN length check) |
| Payload is not valid JSON | HTTP 400 at Webhook level | FR-001 | **PASS** (Rejected at trigger level) |
